...
func parseLS1(data []byte) LS1 {
	...
	l.NumOfLinks = binary.BigEndian.Uint16(u.ShiftN(data, &o, 2))

	for i := 0; i < int(l.NumOfLinks); i++ {
		link := LS1Link{
			LinkID:     net.IP(u.ShiftN(data, &o, 4)),
			Data:       net.IP(u.ShiftN(data, &o, 4)),
			Type:       LS1LinkType(u.ShiftOne(data, &o)),
			MetricsNum: uint8(u.ShiftOne(data, &o)),
			Metric:     binary.BigEndian.Uint16(u.ShiftN(data, &o, 2)),
		}
